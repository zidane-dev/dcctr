...
trait AuthenticatesUsers
{
    ...
    protected function authenticated(Request $request, $user)
    {
        ...
        $domaine_id = $user->domaine->id;
        $request->session()->put('domaine_id', $domaine_id);
        $domaines = (new DomaineController)->get_domaines();
        $request->session()->put('domaine_in', $domaines);
        if($userRole != 's-a'){
            $etat = (new UserValidationController)->get_supposed_states($userRole);
            $request->session()->put('etat', $etat);
            $rejet = (new UserValidationController)->get_reject_states($userRole);
            (new BadgeController)->refresh_badges($request);
            $request->session()->put('rejet', $rejet);
        }
    }
